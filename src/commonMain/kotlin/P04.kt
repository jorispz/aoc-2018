val p04 = fun() {
    val lines = input04.lines().map { it.split(Regex("\\s")) }
    ...
    // 186
    lines.filter { line ->
        line.map { it.sorted() }.distinct().count() == line.count()
    }.count().print()
}

val input04 = """bdwdjjo avricm cjbmj ran lmfsom ivsof
...
jwfm ptjwrbl hhuv uolz adyweh qpj wxyogp igvnojq jmfw pqs fsnirby""".trimIndent()